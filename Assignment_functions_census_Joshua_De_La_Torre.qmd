...
df2<- c(2,4,6,8,10) 
df2result <- function(df2result){
  result <- paste0("The mean is ", mean(df2))
  return(result)
}
df2result(result)
...
jfkflights<- flights
filtered_data<- filter(jfkflights, origin == "JFK")
filtered_data2<- jfkflights[which(jfkflights$origin == "JFK"),]
head(filtered_data)
...
agg = aggregate(filtered_data,
                by = list(filtered_data$carrier),
                FUN = 'order')
head(filtered_data)
...
```{r}
#census_api_key("60eaa45d650b8178b5b376a7cc53499545ee6d36", install=TRUE)

#census_api_key("60eaa45d650b8178b5b376a7cc53499545ee6d36", overwrite = TRUE, install=TRUE)
...
get_acs(geography = "tract", state = NULL)
